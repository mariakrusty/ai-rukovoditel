...
import { Neg, NegSum, Pos } from '../components/Signals'
import Reveal from '../components/Reveal'
import AssistantFeed from '../components/AssistantFeed'

const PAIRS: { was: ReactNode; now: ReactNode }[] = [
  {
    was: '«Эквадор берём у своих. Так привыкли, и человек хороший»',
    now: (
      <>
        Та же роза, та же длина, та же плантация — у поставщика в центре{' '}
        <Pos>на 10 ₽ дешевле</Pos>. На поставке в 500 стеблей это{' '}
        <NegSum>5 000 ₽</NegSum>. Каждую неделю
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    now: (
      <>
        Цветок 2 100, упаковка 300, доставка 400, эквайринг 100, скидка 500.
        Осталось <Pos>1 600</Pos>, <NegSum>а не 5 000</NegSum>
      </>
    ),
  },
  {
    ...
    now: (
      <>
        Платим в последний день отсрочки, деньги лежат на накопительном.
        На 300 000 ₽ за две недели — около <Pos>1 800 ₽</Pos> просто за то,
        что полежали
      </>
    ),
  },
  {
    ...
    now: (
      <>
        Время первого ответа считается.{' '}
        <Pos>Видно, в какие часы теряем и сколько это в рублях</Pos>
      </>
    ),
  },
  {
    ...
    now: (
      <>
        Карта процессов: как устроена работа, записано и{' '}
        <Pos>работает без вашего участия</Pos>
      </>
    ),
  },
]
...
// В закрытых строках красного нет; в раскрытой карточке — одна проблема
// (красный) и один ответ (голубой).
const PAIRS_M: {
  title: string
  digit: string
  was: ReactNode
...
}[] = [
  {
    ...
    now: (
      <>
        Та же роза, та же длина, та же плантация — у поставщика в центре{' '}
        <Pos>на 10 ₽ дешевле</Pos>.
      </>
    ),
    result: (
      <>
        На поставке в 500 стеблей это <NegSum>5 000 ₽</NegSum>. Каждую неделю
      </>
    ),
  },
  {
    ...
    was: (
      <>
        «Постоянно <Neg>не хватает красной гвоздики</Neg>. А потом она стоит
        и вянет»
      </>
    ),
    now: 'Точное количество от поставки до поставки, с учётом логистического плеча.',
    result: <Pos>Ни дефицита, ни увядших остатков</Pos>,
  },
  {
  ...
  },
  {
    ...
    was: (
      <>
        «Пришёл счёт — <Neg>оплатила сразу, чтобы не забыть</Neg>»
      </>
    ),
  ...
  },
  {
    ...
    was: (
      <>
        «Написали в директ в девять вечера, <Neg>ответили утром</Neg>»
      </>
    ),
    now: 'Время первого ответа считается.',
    result: <Pos>Видно, в какие часы теряем и сколько это в рублях</Pos>,
  },
  {
    ...
    was: (
      <>
        «Весь магазин у меня в голове. Уехала на три дня —{' '}
        <Neg>всё встало</Neg>»
      </>
    ),
    now: 'Карта процессов: как устроена работа, записано.',
    result: <Pos>Работает без вашего участия</Pos>,
  },
]
            ...
            {PAIRS_M.map((pair, i) => (
              ...
              >
                ...
                <div className="flex flex-col gap-4 px-5 pb-5">
                  ...
                  <p className="border-t border-white/10 pt-3 text-[15px] leading-snug text-white/85">
                    {pair.result}
                  </p>
                  {i === 0 && <AssistantFeed />}
                </div>
